...
count=0
for line in $(cat $(dirname "$PWD")/conf/config.txt); do
  (( count++ ))
	key=${line%%=*}
  value=${line#*=}
	addConfig ${contentType} ${nacosAddr} ${key} ${value%$'\r'}
	
	#curl -X POST -H ${contentType} "http://${nacosAddr}/nacos/v1/cs/configs?dataId=${key}&group=SEATA_GROUP&content=${value%$'\r'}"
done
